...
import { classTypeStyles, countdownLabel, formatCurrency, formatDate, formatTime, startOfWeek, statusBadge, toDateInputValue } from '../lib/format';
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="panel flex flex-col gap-4 p-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 font-display text-4xl font-bold text-slate-900">Welcome back, {user?.name?.split(' ')[0]}</h1>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-2xl bg-slate-950 px-4 py-3 font-semibold text-white transition hover:bg-slate-900"
            >
              <LogOut size={18} />
              Logout
            </button>
        ...
        <section className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
          <div className="space-y-6">
            ...
            <div className="panel p-6">
              ...
              <div className="mt-6 overflow-x-auto">
                <div className="grid min-w-[520px] grid-flow-col grid-rows-7 gap-2">
                  {heatmap.map((day) => (
                    <div
                      key={day.key}
                      title={`${day.label}: ${day.count} classes`}
                      className={`h-7 w-7 rounded-lg ${heatCellTone(day.count)}`}
                    />
                  ))}
                </div>
              </div>
          ...
          <div className="space-y-6">
            <div className="panel p-6">
              ...
              <div className="mt-4 flex items-center justify-between gap-4 rounded-3xl bg-slate-950 p-6 text-white">
                <div>
                ...
                <span className={`rounded-full px-4 py-2 text-xs font-bold uppercase ${statusBadge(member?.status)}`}>
              ...
              <div className="mt-4 space-y-3">
                {(paymentsQuery.data || []).filter((payment) => payment.status !== 'paid').length === 0 ? (
                  ...
                  (paymentsQuery.data || [])
                    ...
                    .map((payment) => (
                      <div key={payment.id} className="flex flex-wrap items-center justify-between rounded-2xl border border-slate-200 p-3">
                        <div>
