  ...
  return (
    <div className="space-y-6">
      <section className="panel p-5 sm:p-6">
        <h1 className="font-display text-2xl font-bold text-slate-900 sm:text-3xl">Trainer Portal</h1>
        <p className="mt-2 text-sm text-slate-600 sm:text-base">Hello {user?.name}, here's your coaching dashboard.</p>
      </section>
              ...
              {schedule.map((session) => (
                <div key={`${session.id}-${session.start_at}`} className="rounded-2xl border border-slate-200 p-4">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <span className={`inline-flex rounded-full px-3 py-1 text-xs font-bold uppercase ${statusBadge('confirmed')}`}>
                      Confirmed
                    </span>
                  </div>
                </div>
                ...
                <div key={booking.id} className="rounded-2xl border border-slate-200 p-4">
                  <div className="flex flex-col gap-3">
                    <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
                      ...
                      <span className={`inline-flex rounded-full px-2 py-1 text-xs font-bold uppercase ${statusBadge(booking.status)}`}>
                        {booking.status}
                      </span>
                    </div>
                  </div>
      ...
      <section className="panel p-5 sm:p-6">
        <button
          ...
          className="w-full rounded-2xl bg-yellow-400 px-4 py-3 font-bold text-slate-900 transition hover:bg-yellow-300 sm:w-auto"
        >
