...
export default function RegisterPage() {
  ...
  const registerMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      const destination = data.user.role === 'owner' ? '/dashboard' : data.user.role === 'trainer' ? '/trainer-portal' : '/member-portal';
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="grid min-h-screen lg:grid-cols-[1.1fr_0.9fr]">
        <section className="relative overflow-hidden border-b border-white/10 p-8 sm:p-10 lg:border-b-0 lg:border-r">
          ...
          <div className="relative flex h-full flex-col justify-between">
            <div>
              <div className="flex items-center gap-3">
                <div className="flex h-14 w-14 items-center justify-center rounded-3xl bg-accent text-2xl text-slate-950">🏋️</div>
                <div>
              ...
              <div className="max-w-xl py-14">
                <h1 className="font-display text-5xl font-bold leading-tight sm:text-6xl">Create your fitness portal account</h1>
                <p className="mt-4 text-lg text-slate-300">Register for member access and start booking classes right away.</p>
              </div>
            </div>
            <div className="text-sm text-slate-400">Demo hints: use owner/member/trainer logins from the login page once you’re registered.</div>
          </div>
        </section>

        <section className="flex items-center justify-center p-6 sm:p-10">
          <div className="w-full max-w-md rounded-[2rem] border border-white/10 bg-white p-8 text-slate-900 shadow-2xl shadow-slate-950/30">
            <div className="mb-8">
              ...
              <p className="mt-2 text-slate-500">Fill in your details to create member access (members only for now).</p>
            </div>
